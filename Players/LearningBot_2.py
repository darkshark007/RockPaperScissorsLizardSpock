...
class LearningBot(Player):
	...
	def __init__ (self):
		Player.__init__(self,"LearningBot_2");
		
		# Build the Outcome table structure using a pseudo-array list
	...
	def Play(self):
		if ( self._myCurrentMove == 0):
			...
			if ( self._myLastMove == 0 ):
				...
				for i in range(5):
					for j in range(5):
						...
						moveStrength[j] += self._outcomeTable[self._myLastMove.getInt()][i][j] * 2.00 # Loop over all the possible combinations based on this bots last pick
					moveStrength[j] += self._outcomeTable[self._myLastMove.getInt()][self._myOpponent._myLastMove.getInt()][j] * 4.00 # Loop over all the possible combinations based on this bots and the opponents last pick
				...
				highestV = moveStrength[highest]
				moveStrength[highest] = 0
				highest2 = r.randint(0,4)
				for i in range(5):
					if ( moveStrength[i] > moveStrength[highest2] ):
						highest2 = i
				...
				#print("Selected "+str(CM)+": "+str(highestV)+"     and "+str(CM2)+": "+str(moveStrength[highest2]))

			# Next, now that i have the opponents likely choice, decide on a move that will win.
			...
			while (self._myCurrentMove.compareTo(CM)[1] == "Lose") | (self._myCurrentMove.compareTo(CM2)[1] == "Lose"):
				myChoice = r.randint(1,5)
				self._myCurrentMove = {
					1: self.pRock,
					2: self.pPaper,
